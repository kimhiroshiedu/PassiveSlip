function[dR2Sxz,dR2Syz]=makeG_d_02(triC,tri3)
...
sitaD=2.0*pi-Rdip;
 ...
 for i=1:n%numbers of fault%
  ...
  dR2Sxz(:,i)=-s2*(c1*s1*(Syy(:,i)-Sxx(:,i))+(c1^2-s1^2)*Sxy(:,i))+c2*(c1*Sxz(:,i)+s1*Syz(:,i));
  %R2Syy(:,i)=c2^2*(s1^2*Sxx-2*s1*c1*Sxy+c1^2*Syy)+2*c2*s2*(c1*Syz-s1*Sxz)+s2^2*Szz;
  dR2Syz(:,i)=c2*s2*(Szz(:,i)-s1^2*Sxx(:,i)+2*s1*c1*Sxy(:,i)-c1^2*Syy(:,i))+(c2^2-s2^2)*(c1*Syz(:,i)-s1*Sxz(:,i));
